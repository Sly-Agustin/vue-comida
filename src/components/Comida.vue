<template>
    <div class="fontRoboto">
        <hr>
        <h1>Soy el titulo de comida</h1>
        <p>Soy parte del cuerpo de comida</p>
        <p>Probando si la propiedad viene del padre: {{ nombre }}</p>
        <div id="divIngredientes">
            <label id="testTextLabel">Campo de texto de prueba</label>
            <input type="text" id="testText" name="testText" class="form-control" placeholder="Ingrese valor">
            <input type="button" class="btn btn-primary" value="Agregar" id="botonIngredientes" @click="verQueHayEnCampos()">
            <!--<button type="button" class="btn btn-primary" @click="verQueHayEnCampos()">
                Ver campos
            </button>-->
        </div>
        <p>Valor de etiqueta inicial (se puede cambiar con el campo de abajo): {{ etiquetaACambiar }}</p>
        <!--vmodel bindea el input a etiquetaACambiar, cada vez que se modifique el campo se modifica la variable
        etiquetaACambiar-->
        <input v-model="etiquetaACambiar"/>
